...
def chunks(target: list, count: int):
    """This function takes a list and an integer as input and yields
    a generator object.

    The generator yields sub-lists of the input list with a maximum length
    equal to the input integer value.
    The purpose of this function is to split a large list into smaller
    sub-lists, each with a maximum number of elements equal to count.
    """
...
async def delete_or_edit_message(chat_id: int, message_id: int, bot: Bot):
    with suppress(TelegramBadRequest):
        if not await bot.delete_message(
                chat_id=chat_id,
                message_id=message_id
        ):
