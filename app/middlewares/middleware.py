...
class DynamicDataProviderMiddleware(BaseMiddleware):
    ...
    def __init__(self):
        self.dynamic_data = CONFIG.read_yaml(file_path=os.path.relpath('bot_data.yaml'))
    ...
    async def __call__(self, handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]],
        ...
        if event.chat.type == ChatType.SUPERGROUP:
            ...
            if thread_id := event.message_thread_id:
                if CONFIG.bots[bot.id].thread_id == thread_id or not CONFIG.bots[bot.id].thread_id:
                    return await handler(event, data)
        else:
            return await handler(event, data)
