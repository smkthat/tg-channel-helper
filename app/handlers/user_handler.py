...
async def user_ask(m: Message, bot: Bot, dynamic_data: dict, user_service: UserService) -> None:
    try:
        forwarded = await m.forward(
            chat_id=CONFIG.bots[bot.id].admin_chat_id,
            message_thread_id=CONFIG.bots[bot.id].thread_id
        )
        await user_service.set_user_link(bot.id, forwarded.message_id, m.from_user.id)
        if dynamic_data['answer_message']:
            await m.reply(
                text=dynamic_data['answer_message']['text']
            )
    except TelegramBadRequest:
        msg = f'admin chat not found! please add bot to admin chat id={CONFIG.bots[bot.id].admin_chat_id}'
        LOGGER.error(msg)


@media_group_handler
async def answer_to_user_album(messages: List[Message], bot: Bot, user_service: UserService) -> None:
    first_message = messages[0]
    reply_message = first_message.reply_to_message
    if reply_message.forward_date:
        if user_id := await user_service.get_user(bot.id, reply_message.message_id):
            for m in messages:
                await m.copy_to(chat_id=user_id)
        else:
            await first_message.answer('⚠️ Пользователь не найден, сообщение не доставлено')


async def answer_to_user(m: Message, bot: Bot, user_service: UserService) -> None:
...
def register_user_handlers(dp: Dispatcher):
    dp.message.register(
        ...
        lambda message: message.reply_to_message and message.reply_to_message.forward_date,
