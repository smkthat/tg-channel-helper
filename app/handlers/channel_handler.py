...
from app.halpers.utils import add_ui_messages, delete_ui_messages, delete_or_edit_message
...
async def create_message(c: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await c.answer(
        text='! В сообщении может быть не больше одного медиа файла'
             '\n\nК сообщению будет прикреплена кнопка "Прислать новость" с ссылкой на чат с ботом',
        show_alert=True
    )
...
@media_group_handler
async def input_created_message_album(messages: List[Message], bot: Bot, state: FSMContext):
    m = messages.pop(0)
    for message in messages:
        await delete_or_edit_message(chat_id=message.chat.id, message_id=message.message_id, bot=bot)
...
async def input_created_message(m: Message, bot: Bot, state: FSMContext):
    ...
    new_message = await m.copy_to(
        chat_id=m.chat.id,
        reply_markup=get_bot_link_keyboard(f'https://t.me/{bot_user.username}?start=')
    )
...
async def accept_create_message(c: CallbackQuery, callback_data: AdminCallback, bot: Bot, state: FSMContext):
        ...
        match callback_data.data:
            case 'yes':
                if channel_id := CONFIG.bots[bot.id].channel_id:
                    ...
                    await c.answer(text='Сообщение отправлено на канал', show_alert=True)
                else:
                    await c.answer(text='❌ Не удалось определить канал', show_alert=True)
            case 'no':
                await c.answer(text='❌ Вы отменили отправку сообщения')

    await delete_or_edit_message(chat_id=c.message.chat.id, message_id=data['tmp_message_id'], bot=bot)
    await delete_or_edit_message(chat_id=c.message.chat.id, message_id=c.message.message_id, bot=bot)
...
def register_channel_handlers(dp: Dispatcher):
    ...
    dp.message.register(
        ...
        lambda message: message.content_type in [ContentType.PHOTO, ContentType.VIDEO, ContentType.DOCUMENT],
